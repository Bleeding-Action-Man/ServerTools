...
function ShowPanel(bool bShow) {
	...
	if (bShow) {
			...
			if (class'KFServerTools'.default.bAdminAndSelectPlayers)
				b_ApplyButton2.Caption = AdminsOnlyText;
...
function bool ClickOfAButton(GUIComponent Sender) {
	...
	if (Sender == b_ApplyButton){
		UpdateDefaultTraderTime(PC);
	}

	if (Sender == b_ApplyButton2){
		UpdateCurrentTraderTime(PC);
	}

	if (Sender == b_ShowDeadPlayers){
		PrintDeadPlayers(PC);
	}

	if (Sender == b_Revive){
		RevivePlayerByCode(PC);
	}
